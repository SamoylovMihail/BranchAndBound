...
public class TSP {
	...
	public void generateSolution() {
		...
		System.out.println("Elapsed time for entire algorithm: " + t.getElapsedTime() + " seconds.");
		System.out.println();
	}
	...
	private void branchAndBound(Node node, int edgeIndex) {
		if (Integer.parseInt(t.getElapsedTime()) >= Main.maxWorkTime) {
			System.out.println(
					"\n\nBest final tour cost: " + bestTour + "\nBest final tour: " + bestNode.tour());
			System.exit(0);
		}
			...
			if (node.isTour()) {
				...
				if (node.tourCost() < bestTour) {
					...
					System.out.println("\n\nBest tour cost so far: " + bestTour + "\nBest tour so far: "
							...
							+ "\nElapsed time to date for branch and bound: " + t.getElapsedTime() + " seconds.\n");
				}
			} else {
				if (node.lowerBound() < 2 * bestTour) {
					...
					if (newNodeCount % 10000 == 0 && bestNode != null) {
						System.out.println(
								"\n\nBest tour cost so far: " + bestTour + "\nBest tour so far: " + bestNode.tour());
					}
